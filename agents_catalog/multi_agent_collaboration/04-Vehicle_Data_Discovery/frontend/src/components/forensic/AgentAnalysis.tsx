 ...
 * 
 * Renders structured output from AI agents including scene understanding,
 * behavioral analysis, safety assessment, and business intelligence.
 */
"use client"
...
export default function AgentAnalysis({ sceneData }: AgentAnalysisProps) {
  ...
  const cleanAgentText = (value: unknown): string => {
  ...
  // Debug logging  // Show fallback UI when no agent analysis data is available
  if (hasNoData) {
    return (
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {Object.entries(sceneData.anomaly_analysis.metrics).map(([key, value]) => {
                    ...
                    const info = (metricInfo as Record<string, { label: string; description: string; format: (val: number) => string; color: (val: number) => string }>)[key] || {
