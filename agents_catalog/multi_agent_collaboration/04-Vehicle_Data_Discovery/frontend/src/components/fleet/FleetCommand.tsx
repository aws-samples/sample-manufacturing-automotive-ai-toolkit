/**
 * FleetCommand - Main Dashboard Component
 * 
 * Primary dashboard view displaying scene grid/list, search bar,
 * filter pills, metrics ribbon, and pagination controls.
 */
"use client"
    ...
    <div className="space-y-6">
      ...
      {loading && !data ? (
        ...
        <>
          ...
          {viewMode === "grid" ? (
            <SceneGrid scenes={data?.scenes || []} isLoading={loading} />
          ) : (
            data?.scenes?.length === 0 ? (
              ...
              <motion.div
                ...
                className="space-y-3"
              >
                {data?.scenes?.map((scene: SceneData, index: number) => (
                  <SceneListItem
                    key={scene.scene_id}
                    scene={scene}
                    index={index}
