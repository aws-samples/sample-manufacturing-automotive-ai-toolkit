/**
 * useSceneDetail - Individual scene data fetching hook
 * 
 * Fetches comprehensive data for a single driving scene including
 * video URLs, agent analysis results, and behavioral metrics.
 * 
 * @param {string} sceneId - Scene identifier (e.g., "scene-0001")
 * @returns {Object} Scene detail state
 * @returns {SceneDetail | null} data - Complete scene data
 * @returns {boolean} loading - Loading state
 * @returns {string | null} error - Error message if fetch failed
 * 
 * @example
 * const { data, loading, error } = useSceneDetail("scene-0001")
 * if (data) {
 *   console.log(data.video_url, data.agent_analysis)
 * }
 * 
 * API Endpoint:
 * - GET /scene/{sceneId}
  ...
  async function fetchSceneDetail() {
    ...
    try {
      ...
      const extractTags = () => {
        ...
        const sceneCharacteristics = (sceneAnalysisRaw as Record<string, unknown>).scene_characteristics as Record<string, unknown> || {}
