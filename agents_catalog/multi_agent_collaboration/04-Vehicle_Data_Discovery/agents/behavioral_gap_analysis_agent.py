#!/usr/bin/env python3
"""
Fleet Behavioral Gap Analysis Agent - AgentCore Runtime Version
...
CRITICAL: Return a structured response object (NOT a JSON string) with these exact fields populated based on your scene understanding analysis:

Required response structure - return the actual object with these fields:
- scene_analysis: dict containing environmental_conditions, traffic_complexity, vehicle_behavior, interaction_patterns, infrastructure_context
- scene_characteristics: dict containing scenario_type, complexity_level, safety_criticality, training_relevance  
- behavioral_insights: dict containing critical_decisions, edge_case_elements, performance_indicators, learning_opportunities
- recommendations: dict containing hil_testing_priority, similar_scenario_needs, model_training_focus, regulatory_considerations
- confidence_metrics: dict containing overall_confidence (float between 0.0-1.0), analysis_depth (string)
...
- ONLY use information provided in the scene data
...
def invoke(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    global scene_understanding_agent  # Access the global variable

    try:
        # --- LAZY INITIALIZATION FIX ---
        if scene_understanding_agent is None:
            logger.info("Agent not initialized. Initializing now (Lazy Loading)...")
            success = initialize_scene_understanding_agent()
            if not success:
                raise RuntimeError("Failed to lazily initialize Scene Understanding agent")
        # -------------------------------
...
def prepare_scene_understanding_context(payload: Dict[str, Any]) -> str:
    ...
    # Extract anomaly context for discovery mode analysis
    ...
    # Extract behavioral insights from dense scene descriptions AND Phase 3 visual analysis
    # Get rich Phase 3 visual analysis text for environmental context
    phase3_visual_analysis = payload.get("behavioral_analysis_text", "")
...
## Phase 3 Visual Scene Analysis (Environmental Context):
{phase3_visual_analysis or "No Phase 3 visual analysis available"}

## Behavioral Metrics from Previous Phases:
{json.dumps(behavioral_metrics, indent=2)}

## KEY METRICS SUMMARY (VALIDATED):
...
2. **Environmental Analysis**: Extract weather conditions, lighting, and road conditions from the Phase 3 Visual Scene Analysis text provided above
...
def parse_scene_understanding_response(response_text: str) -> SceneUnderstandingResult:
    """Parse agent response into structured scene understanding analysis result with robust markdown handling"""
    ...
    try:
        ...
        # Step 4: Try progressive fallback strategies
        try:
            # Try to extract just the content between first and last braces more carefully
            import re
            json_pattern = r'\{.*\}'
            matches = re.findall(json_pattern, response_text, re.DOTALL)
            if matches:
                # Take the longest match (most complete JSON)
    ...
    logger.warning("Using text-based fallback due to parsing failure")
    return SceneUnderstandingResult(
...
if __name__ == "__main__":
    logger.info("Starting Scene Understanding Agent with AgentCore Runtime")
    app.run()