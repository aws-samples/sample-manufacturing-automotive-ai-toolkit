#!/usr/bin/env python3
"""
Fleet Discovery Studio - Fleet S3 Vectors Behavioral Embeddings (Phase 4-5)
...
def prepare_embedding_inputs(behavioral_analysis: Dict[str, Any], scene_id: str) -> List[Dict[str, Any]]:
    ...
    if scene_description and scene_description.strip():
        embedding_inputs.append({
            ...
            "text": f"Behavioral insight for Fleet scene {scene_id}: {scene_description}"
        })
    ...
    if comprehensive_analysis and comprehensive_analysis.strip():
        embedding_inputs.append({
            ...
            "text": f"Scene understanding for Fleet scene {scene_id}: {comprehensive_analysis}"
        })
    ...
    if analysis_quality and analysis_quality.strip():
        embedding_inputs.append({
            "type": "scene_context",
            "id": "analysis_quality",
            "text": f"Scene context for Fleet scene {scene_id} (analysis_quality): {analysis_quality}"
        })

    # SUCCESS: CREATE COMBINED ANALYSIS using actual content
    if scene_description or comprehensive_analysis:
        combined_text = f"Complete behavioral analysis for Fleet scene {scene_id}. "
