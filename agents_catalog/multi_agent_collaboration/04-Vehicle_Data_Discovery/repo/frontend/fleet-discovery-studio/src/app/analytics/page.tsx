...
export default function AnalyticsPage() {
  ...
  console.log(" AnalyticsPage - Hook returned:", {
    data: data ? "loaded" : "null",
    coverageData: coverageData ? "loaded" : "null",
    loading
  })
  ...
  const pieData = trafficLightData.stats ? [
    {
      name: "🔴 Critical",
      value: trafficLightData.stats.critical.count,
      color: "#EF4444"
    },
    {
      name: "🟡 Deviation",
      value: trafficLightData.stats.deviation.count,
      color: "#F59E0B"
    },
    {
      name: "🟢 Normal",
      value: trafficLightData.stats.normal.count,
      color: "#10B981"
    }
  ].filter(item => item.value > 0) : [] // Only show categories with scenes

  const riskTimelineData = data?.risk_timeline?.map((item: any) => ({
    date: item.date,
    risk: Math.round((item.risk_score || item.risk) * 100),
    scene: item.scene_id || item.id
  })) || []

  console.log(' CHART DATA DEBUG:')
  console.log('  - pieData:', pieData)
  console.log('  - riskTimelineData:', riskTimelineData)
  console.log('  - trafficLightData.stats:', trafficLightData.stats)

  // Extract hybrid coverage matrix data
  const industryCategories = coverageData?.coverage_matrix?.industry_standard_categories || []
  const discoveredCategories = coverageData?.coverage_matrix?.discovered_categories || []
  const allCoverageItems = [...industryCategories, ...discoveredCategories]
  const coverageAnalysis = coverageData?.coverage_matrix?.coverage_analysis || {}
  ...
  const handleCoverageMatrixSimilar = (category: any) => {
    try {
      ...
      console.error('Coverage Matrix Find Similar error:', error)
      // Fallback to simple search
      window.location.href = `/search/results?q=${encodeURIComponent(category.category)}`
    }
  }

  // ODD Discovery Find Similar handler (scene-based twin-engine search)
  const handleOddDiscoverySimilar = (category: any) => {
    try {
      const searchParams = new URLSearchParams({
      ...
      if (category.representative_scene_id) {
        ...
        console.log(`Using representative scene ${category.representative_scene_id} for category: ${category.category}`)
      } else {
        ...
        searchParams.set('auto_query', 'true')
        console.log(`Using text-based query for category: ${category.category} (no representative scene available)`)
      }

      window.location.href = `/search/results?${searchParams.toString()}`
    } catch (error) {
      console.error('ODD Discovery Find Similar error:', error)
      // Fallback to simple search with category name
      window.location.href = `/search/results?q=${encodeURIComponent(category.category)}`
    }
  }
  ...
  return (
    <ProtectedRoute>
      ...
      <Card className="p-8">
        ...
        {coverageAnalysis?.total_scenes_analyzed > 0 && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {allCoverageItems.map((item: any, index: number) => (
            <motion.div
              key={item.category}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-800">
                    ${oddDiscoveryData.data.dto_savings_estimate.estimated_savings_usd.toLocaleString()}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-800">
                    {oddDiscoveryData.data.dto_savings_estimate.efficiency_gain_percent.toFixed(1)}%
                  </div>
                  <div className="flex items-center justify-center gap-1 text-sm text-green-600">
                ...
                <div className="text-center">
                  <div className="text-lg font-medium text-gray-700">
                    ${oddDiscoveryData.data.dto_savings_estimate.naive_cost_usd.toLocaleString()}
                  </div>
                ...
                <div className="text-center">
                  <div className="text-lg font-medium text-green-700">
                    ${oddDiscoveryData.data.dto_savings_estimate.intelligent_cost_usd.toLocaleString()}
                  </div>
                  <div className="flex items-center justify-center gap-1 text-sm text-green-600">
              ...
              <div className="mt-4 text-center">
                <div className="inline-block">
                  <Badge className="bg-green-200 border-green-300 text-green-800 px-4 py-2 flex items-center gap-2">
                    <span>🧠 Uniqueness Ratio: {(oddDiscoveryData.data.overall_uniqueness_ratio * 100).toFixed(1)}%
                    ({oddDiscoveryData.data.total_unique_scenes_estimated.toFixed(1)}/{oddDiscoveryData.data.total_scenes_analyzed} scenes)</span>
                    <InfoTooltip
