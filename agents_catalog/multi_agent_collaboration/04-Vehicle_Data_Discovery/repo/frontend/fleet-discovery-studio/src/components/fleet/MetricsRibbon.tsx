...
import { TrendingUp, AlertTriangle, DollarSign, Activity, AlertCircle, Clock, CheckCircle } from "lucide-react"
...
export default function MetricsRibbon({ initialStats }: MetricsRibbonProps = {}) {
  // Use server-side data if provided, otherwise fallback to hooks
  const hookData = useFleetStats()
  const trafficLightData = useTrafficLightStats()
  const stats = initialStats || hookData.stats
  const loading = (initialStats ? false : hookData.loading) || trafficLightData.loading
