...
export default function SearchResultsContent() {
  ...
  useEffect(() => {
    ...
    console.log('SearchResultsContent - URL params:', { query, mode, refScene, hasSearched })

    if (!hasSearched) {
      // LOGIC BRANCH 1: Visual Search (Scene-to-Scene)
      if (refScene && mode === 'visual') {
        console.log('Triggering visual search for scene:', refScene)
        search("", { mode: 'visual', sceneId: refScene })
        setHasSearched(true)
      } 
      // LOGIC BRANCH 2: Text Search (Twin Engine)
      else if (query) {
        console.log('Triggering text search for query:', query)
  ...
  console.log('🔍 SearchResultsContent - Raw results:', { 
  ...
  console.log('🎯 SearchResultsContent - After filtering:', { 
    relevantCount: relevantResults.length,
    threshold: SIMILARITY_THRESHOLD,
    firstRelevant: relevantResults[0]
  })
  ...
  const transformedResults = searchResults.map(result => {
    ...
    const cameraNames = cameras.map((c: any) => c.camera).join(", ") || "CAM_FRONT"
