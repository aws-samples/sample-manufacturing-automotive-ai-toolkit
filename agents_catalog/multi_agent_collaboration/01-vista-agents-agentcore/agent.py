...
app = FastAPI(title="VISTA Agent Server", version="1.0.0")
...
When a customer asks about finding an appointment date and time use the tool find_appointment_slots and return the data in this format. Include the information about calendar conflict if available:
{
...
4. Present the available slots in the JSON format listed above
...
def check_calendar_conflicts(appointment_slots):
    ...
    try:
        ...
        calendar_id = 'your-email-here@gmail.com'
...
class InvocationRequest(BaseModel):
    ...
    # Make input optional with a default empty dict
...
async def invoke_agent(request: Request):
    try:
        ...
        # Parse JSON manually regardless of content type
        try:
        ...
        # Extract prompt and session_id from various possible locations
        prompt = None
        ...
        # Check top level
        if prompt is None and "prompt" in request_data:
        ...
        if not prompt:
            error_msg = "No prompt found in request. Please provide a 'prompt' field."
            return JSONResponse(
                status_code=400,
        ...
        logger.info(f"Processing request with session_id: {session_id}")
        
        # Configuration constants
        DYNAMO_DB_CONVERSATION_HISTORY_TABLE = 'Strands_Conversation_History'
        ...
        # Initialize agent with conversation history for this request
        agent = Agent(
            model=bedrock_model,
            system_prompt=SYSTEM_PROMPT,
            tools=[
                find_nearest_dealerships,
                diagnose_vehicle_issues,
        ...
        return JSONResponse(
            status_code=500,
            content={"detail": f"Agent processing failed: {str(e)}"}
        )
