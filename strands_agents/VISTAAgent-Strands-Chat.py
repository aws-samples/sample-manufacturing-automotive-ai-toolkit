...
You can help customers find the nearest authorized dealerships in a city, diagnose vehicle issues using VIN or customer ID, find service appointments and 
find parts inventory by name.
...
2. If you get a valid JSON response from the tool, just simply return the dealers including all the data. Do NOT add any other dealer information, and just pass the output from the tool find_nearest_dealerships.
...
3. Send a repsonse that the appointment has been booked successfully.
...
When a customer asks about finding an appointment date and time use the tool find_appointment_slots and return available appointment times. Include the information about calendar conflict if available:
...
When a customer asks about a particular parts being in stock, then use the tool get_parts_by_name to get the current inventory. 
...
def get_parts_by_name(part_name: str) -> str:
    ...
    try:
        ...
        response = table.scan(
            FilterExpression=boto3.dynamodb.conditions.Attr('Custom_Name').contains(part_name.lower())
        )
...
def handler(event: Dict[str, Any], _context) -> str:
    ...
    try:
        ...
        agent = Agent(
            ...
            tools=[find_nearest_dealerships, diagnose_vehicle_issues, find_appointment_slots, book_appointment, 
                   cancel_appointment, get_customer_appointments, get_parts_by_name],
